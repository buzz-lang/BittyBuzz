#include "bbz_kilobot_controller.h"
...
BBZ_KilobotController::BBZ_KilobotController() :
        m_pcMotors(NULL),
        m_pcLED(NULL),
        m_pcLight(NULL),
        m_pcCommA(NULL),
...
void BBZ_KilobotController::Init(TConfigurationNode& t_tree) {
    try {
        /* Initialize devices */
        ...
        std::string strBBO;
        GetNodeAttribute(t_tree, "bbo", strBBO);
        printf("bbo file: %s\n", strBBO.c_str());
...
void BBZ_KilobotController::ControlStep() {
    /* Set light reading */
...
void BBZ_KilobotController::Reset() {
...
void BBZ_KilobotController::Destroy() {
...
REGISTER_CONTROLLER(BBZ_KilobotController, "bbzkilobot_controller");